...
Estimates haplotypic or component pairwise linkage disequilibrium (LD), using either genotypes or genotype likelihoods. Estimates are returned with standard errors. These estimates and standard errors can be used for shrinkage estimation. The methods are described in Gerard (2020).

The main functions are:

- `ldest()`: Estimates pairwise LD.
- `mldest()`: Iteratively apply `ldest()` across many pairs of SNPs.
- `ldshrink()`: For shrinking correlation estimates using adaptive shrinkage (Stephens, 2017).
