---
title: "VariantAnnotation-updog-ldsep Pipeline from VCF"
...
To genotype these individuals, we will use the `multidog()` function from the `updog` R package (Gerard et. al., 2018; Gerard and Ferrão, 2020). Make sure that your version of `updog` is at least 2.0.2.

```{r}
packageVersion("updog")
```

We will use `model = "norm"`, using 2 cores to genotype all 20 SNPs.
```{r}
ploidy <- 4
...
```{r}
like_ld <- mldest(geno = larray, K = ploidy, nc = 2, type = "comp")
```
...
```{r}
mom_ld <- mldest(geno = pmmat, K = ploidy, nc = 2, type = "comp")
```
